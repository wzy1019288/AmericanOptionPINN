...
def train_sol_model(
    ...
    for i, (data_sol_intrr, 
        ...
        for _ in range(config.steps_fb_per_pde - 1):
            ...
            x_sol_intrr_gt_Bt = x_sol_intrr[ x_sol_intrr > s_values3 ].view(-1,1)    # 大于B(t)的内点，S取值
            t_sol_intrr_gt_Bt = t_sol_intrr[ x_sol_intrr > s_values3 ].view(-1,1)    # 大于B(t)的内点，t取值
            ...
            fb_dir_target = torch.relu(torch.ones_like(s_values1) * K - s_values1)
            ...
            #--------------- Compute PINN losses
            sol_intrr_NN_gradS = torch.autograd.grad(outputs=sol_intrr_NN, inputs=x_sol_intrr_gt_Bt, grad_outputs=torch.ones_like(sol_intrr_NN), retain_graph=True, create_graph=True)[0]
...
def train_fb_model(
    ...
    for i, (data_sol_intrr, 
        ...
        for _ in range(0, config.steps_fb_per_pde+1, -1):
            ...
            fb_neu_NN_gradS = torch.autograd.grad(outputs=fb_neu_NN, inputs=s_values2, grad_outputs=torch.ones_like(fb_neu_NN), create_graph=True)[0]
            fb_neu_loss = torch.mean((fb_neu_NN_gradS - y_fb_neu)**2)
...
def get_test_data(
    ...
    for i, (data_sol_intrr, 
        ...
        return (x_sol_intrr, t_sol_intrr, x_sol_init, y_sol_init, x_sol_dir, y_sol_dir,
                x_fb_init, y_fb_init, x_fb_dir, x_fb_neu, y_fb_neu)
...
def test(
    ...
    (x_sol_intrr, t_sol_intrr, x_sol_init, y_sol_init, x_sol_dir, y_sol_dir,
        x_fb_init, y_fb_init, x_fb_dir, x_fb_neu, y_fb_neu) = get_test_data(
